...
# works the same as allocate pics, but only uses the headword, not
# the words in the dictionary entry
    
if($#ARGV <= 0) {
    print "Usage: perl restrict_pics.pl <xml dictionary> <results file>\n\n";
    exit 0;
}
...
open(RES, ">results.log") || die "$0: can't open results.log: $!\n";
...
while(<FILE>) {
	...
	$word =~ s/\n//g;
        ...
        $definition = $entry;
        if($definition =~ m/\<GL\>(.+?)\<\/GL\>/) {
            $definition = $1;
            $definition =~ s/\<.+?\>/  /sg;
        } else { 
            $definition = "";
        }
        $entry = $gloss.' # '.$definition;
        $entry =~ s/\s+/ /sg;
        print RES "$word: $entry\n";
        if($opt) {
